...
public class ShellExe {
    ...
    public static int execCommand(String[] command) throws IOException {
        ...
        Log.d(TAG, "execCommand: proc :" + proc+command);
    ...
    public static int execCommandErrorStream(String[] command) throws IOException {
        int result = RESULT_FAIL;
        Runtime runtime = Runtime.getRuntime();
        Process proc = runtime.exec(command);
        BufferedReader bufferedReader = null;
        ...
        try {
            bufferedReader = new BufferedReader(new InputStreamReader(proc
                    .getErrorStream(), Charset.defaultCharset()));
            if (proc.waitFor() == 0) {
                String line = bufferedReader.readLine();
                if (line != null) {
                    sResultBuilder.append(line);
