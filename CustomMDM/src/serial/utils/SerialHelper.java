            ...
            while(!isInterrupted()) {
                try
                {
                    if (mInputStream == null) {
                        Log.i("SerialPort", "mInputStream == null");
                        return;
                    }
                    if (mInputStream.available() > 0) {
                        ...
                        //当接收到数据时，sleep 5毫秒（sleep时间自己把握）

                        byte [] tempBuff;
                        int AllCount = mInputStream.available();

                        tempBuff = new byte[AllCount];
                        int size = 0;
                        while (size < AllCount) {
                            size += mInputStream.read(tempBuff, size, AllCount - size);
                        }
                        if (size > 0){
                            Log.d(TAG, "AllCount: " + String.valueOf(AllCount));
                            onDataReceived(tempBuff);
                        }
                    }
