...
interface ICustomAPI {
    ...
    List<String> getCertificates(int type);

    void installCA(String path);

    void uninstallCA(String name);

    int setNetworkWhitelist(in List<String> ipList, in List<String> ssidList);

    int setNetworkBlacklist(in List<String> ipList, in List<String> ssidList);

    int clearNetworkRules();

    int pin_set_direction(int direction);

    int pin_set_output(int level);

    int pin_get_level();

    boolean open(String port, int baud, int data_bits, int parity, int stop_bits);

    boolean close();

    boolean write(inout byte[] bytes);

    int read(inout byte[] bytes);

    int saveWifiConfig(String ssid, String password, int security);

    int simDataSwitch(int index);

    String getPhoneNumber(int slotIndex);
}
